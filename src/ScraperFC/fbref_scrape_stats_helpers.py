...
from bs4 import Tag
from .fbref_helpers import _get_ids_from_table, _get_stats_table_tag, _get_age_mask
...
def _scrape_not_big5_stats(
    ...
    if isinstance(squad_table_tag, Tag):
        squad_df = pd.read_html(StringIO(str(squad_table_tag)))[0].copy()
    ...
    if isinstance(opp_table_tag, Tag):
        opp_df = pd.read_html(StringIO(str(opp_table_tag)))[0].copy()
    ...
    if isinstance(player_table_tag, Tag):
        ...
        ids_aligned = pd.Series(player_ids, index=player_df.index[mask][:len(player_ids)])
...
def _scrape_big5_stats(
    ...
    if isinstance(player_table_tag, Tag):
        ...
        # Add player IDs
        player_ids = _get_ids_from_table(player_table_tag, "player")
        player_df['Player ID'] = player_ids
    elif player_table_tag is None:
