...
class ScraperFC:
    ...
    def get_league_stats(self, league: str, year: int, source: str = "fbref", stat_type: str = "standard", bypass_cache: bool = False) -> pd.DataFrame:
        ...
        if s == "fbref":
            ...
            # scrape_league_tables returns (general, home, away) DataFrames
            tables = self.understat.scrape_league_tables(str(year), matched_name)
            result = tables[0] if isinstance(tables, tuple) else tables
